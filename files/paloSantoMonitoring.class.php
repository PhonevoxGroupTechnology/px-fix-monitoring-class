...
// Obtendo o diretório que está setado em /etc/asterisk/asterisk.conf
$result = trim(shell_exec("cat /etc/asterisk/asterisk.conf | grep -i astspooldir | awk -F'=>' '{print \$2}'| awk '{\$1=\$1;print \$1;}'")).'/monitor';

// Setando esse diretório como o valor base esperado para gravações
define ('DEFAULT_ASTERISK_RECORDING_BASEDIR', "$result");
...
SQL_COND_EXTENSION;
    ...
    function resolveRecordingPath($recordingfile)
    {
        ...
        // $this->console_log("Gravação: $recordingfile");
    ...
    private function _rutaAbsolutaGrabacion($file)
    {
        ...
        // @ADRIAN: Caso o caminho da gravação seja ABSOLUTO, eu vou ignorá-lo por completo, e pegar apenas o último campo, que em casos ideais, deve ser o nome do arquivo de gravação em si. Vou utilizar o caminho de gravações definido em /etc/asterisk/asterisk.conf para consultar a existência da gravação. Não quero ficar dependente de caminhos estáticos, absolutos. A feiura dessa configuração é que não bate com o valor guardado no Banco de Dados, apesar de solucionar o meu problema. Tenha isso em mente.
        if ($file{0} == '/') {
            $nome_arq_gravacao = end(explode('/', $file)); // Vou ignorar o caminho absoluto que há no arquivo, e transformá-lo em relativo.
            $file = $nome_arq_gravacao;
        }
        ...
        if (strpos($file, '/') === FALSE) {
            // $this->console_log("$file : entrando no datedir");
            ...
            // Testando pra ver se a data quebrada na pasta tá certa
            // $this->console_log("test: ".$basedir.$datedir.$file);
